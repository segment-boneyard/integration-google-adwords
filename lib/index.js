...
GoogleAdWords.ensure(function(msg, settings){
  ...
  var conversionId = getConversionId(msg.event(), settings.events);
  var conversionLabel = getConversionLabel(msg.event(), settings.events);
...
GoogleAdWords.prototype.track = function(msg, callback){
  ...
  var payload = mapper.track(msg, events);
  var conversionId = getConversionId(msg.name(), events);
...
function getConversionId (eventName, events) {
  if (events[eventName]) {
